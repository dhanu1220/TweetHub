...
function TweetBox() {
    ...
    const handleTweet = (e) => {
        ...
        if (user?.providerData[0]?.providerId === 'password') {
            fetch(`http://localhost:5000/loggedInUser?email=${email}`) // Replace with your local server URL
                .then(res => res.json())
                .then(data => {
                    setName(data[0]?.name);
        ...
        if (name) {
            ...
            setPost('');
            setImageURL('');
            fetch('http://localhost:5000/post', { // Replace with your local server URL
                method: "POST",
                headers: {
